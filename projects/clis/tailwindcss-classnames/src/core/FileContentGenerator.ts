...
export class FileContentGenerator {
   ...
   */
  constructor(
    generatedClassnames: TAllClassnames,
    configParser: TailwindConfigParser
  ) {
    ...
    return Object.keys(this._generatedClassNames)
      .map((classGroupKey) => {
        return this.generateTypesGroupTemplate(
          this._generatedClassNames[
            classGroupKey as keyof TAllClassnames
          ] as TAllClassnames,
          classGroupKey
        );
      })
  ...
  private utilityFunctionsTemplate = (): string => {
    ...
    const generalCategoriesTemplate = Object.keys(this._generatedClassNames)
      .map((categorykey) => {
        ...
        return (
          comment + `export const ${fnName}: ${fnType} = classnamesLib as any`
        );
      })
      .join('\n');
  ...
  private mainExportStatementsTemplate = (): string => {
    const specificUtilityFunctionsObjectTemplate = Object.keys(
      this._generatedClassNames
    )
      .map((cn) => {
        const subCategoryObj =
          this._generatedClassNames[cn as keyof TAllClassnames];
    ...
    const generalUtilityFunctionsObjectTemplate = Object.keys(
      this._generatedClassNames
    )
  ...
  private generateTypesGroupTemplate = (
    ...
    return (
      generateMembersStatements().join('\n\n') +
      '\n\n' +
      ...
      items
        .flatMap((item) => {
          ...
          return classnamesThatShouldKeepTheDefaultSuffix.map((x) => {
            ...
            const name = shouldKeepDefaultSuffix
              ? item
